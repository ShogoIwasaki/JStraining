...
async function h1() {
  ...
  // 理由：wait3()、logA()、wait2()、logB()、wait1()、logC() の順に実行されるため。
  try {
    await wait3();
    logA();
    await wait2();
...
function h3() {
  ...
  // 解答： “X” が即座にログに出力
  // 理由：Promiseのコンストラクタに渡される関数が非同期関数であり、エラーはPromiseチェーンによって補足され、catchブロックが実行されるため。
