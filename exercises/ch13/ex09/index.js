...
async function i1() {
  // 解答：42と100が出力
  // 理由：wait1が最初に解決され、42がvに代入される。その後wait2によってvが100に更新されるため。
...
async function i2() {
  // 解答：C、B、A、["A", "B", "C"]の出力
  // 理由：すべてのwaitを解決するのを待ち、それぞれのPromiseは指定した時間が経過した後に、指定した文字列を研ぐに出力するため。
...
async function i3() {
  // 解答：Y、42、B、0と出力
  // 理由：wait3でエラーがすぐにcatchブロックで捕捉される（Yと42が出力）。その2秒後にBが出力。
  //       Promise.allは最初のエラー以外は無視されるためwait1のエラーは無視される。その後、await wait3で待たされていたvの出力が行われるため。（vは0に初期化されているので0が出力）
...
async function i5() {
  ...
  // 理由：i5 関数では、wait((5 - i) * 1000).then(() => log(i)) が5回実行されるが、それぞれの Promise が同時に開始されるため、待機時間が長いPromiseから順にログに出力される。ただしp.then(() => log("COMPLETED"))が最初に実行されてしまう。
